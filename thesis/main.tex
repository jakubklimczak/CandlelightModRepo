...
\subsection{Swagger i dokumentacja API}
W projekcie wykorzystałem bibliotekę \texttt{Swashbuckle.AspNetCore}, która integruje framework \textit{Swagger} z aplikacjami opartymi o \textit{ASP.NET Core}. Swagger pozwala na automatyczne generowanie dokumentacji REST API na podstawie atrybutów kontrolerów oraz definicji modeli danych. Wygenerowana dokumentacja dostępna jest w formacie \textit{OpenAPI}\cite{swaggerdocs2025}.Dzięki zastosowaniu Swaggera możliwe było nie tylko wygodne testowanie endpointów podczas pracy deweloperskiej (przy pomocy interaktywnego interfejsu \textit{Swagger UI}), ale również zwiększenie przejrzystości i łatwości w integracji systemu z potencjalnymi zewnętrznymi usługami. Dokumentacja dostępna jest domyślnie pod ścieżką \texttt{/swagger}, a jej struktura aktualizuje się automatycznie wraz z wprowadzaniem zmian w kodzie.

Swagger wspiera dodatkowo opisywanie parametrów, odpowiedzi oraz statusów HTTP za pomocą atrybutów, takich jak \texttt{[ProducesResponseType]}, \texttt{[FromQuery]}, czy \texttt{/// <summary>}. Umożliwia to tworzenie kompletnej, aktualnej i łatwej do utrzymania dokumentacji API bez potrzeby pisania jej ręcznie.
...
W celu zachowania wysokiej jakości kodu źródłowego oraz unikania powstawania tak zwanemu \textit{code smell}owi (czyli wzorców prowadzących do potencjalnych błędów lub utrudniających rozwój projektu), a także aby zachować branżowe standardy, w projekcie zastosowałem kilka narzędzi wspierających automatyczną analizę kodu oraz jego formatowanie.

\subsection{GitHub Actions}
Do automatyzacji procesu sprawdzania jakości kodu wykorzystałem mechanizm \textit{GitHub Actions} \cite{githubactions2025}. Pozwala on na definiowanie tak zwanego workflow - zadań uruchamianych automatycznie przy określonych zdarzeniach (na przykład push, pull request). Dzięki temu możliwe było automatyczne uruchamianie analizy lintingu i testów jednostkowych przy każdej zmianie w kodzie, co pomaga w szybkim wykrywaniu błędów oraz zapewnia spójność kodu w całym zespole.

W przypadku Candlelight, automatycznie uruchamiany jest linting frontendu i testy zarówno frontendu jak i backendu przy każdym pushu i pull requeście.

\cite{githubactions2025}
...
\cite{moqquickstart2025}

\subsection{Coverlet}
//TODO - consider if not remove
\cite{}
...
\begin{center}
    ...
    \captionof{figure}{Struktura warstwowa aplikacji Candlelight}
\end{center}
...
Projekt \texttt{Candlelight.Api} odpowiada za obsługę zapytań HTTP, pełniąc rolę kontrolera dostępowego do logiki aplikacji. Znajdują się tu kontrolery (\texttt{Controllers}), atrybuty do walidacji (\texttt{Attributes}), customowe \texttt{ModelBinders} i pliki statyczne w katalogu \texttt{wwwroot}. API aplikacji Candlelight korzysta z architektury REST (Representational State Transfer), co oznacza, że komunikacja z klientem odbywa się za pomocą standardowych metod protokołu HTTP (GET, POST, PUT, DELETE), a zasoby aplikacji są identyfikowane poprzez unikalne URI i reprezentowane w formacie JSON, zapewniając czytelność, skalowalność oraz łatwość integracji z frontendem i innymi usługami. Została ona zaimplementowana przy użyciu frameworka ASP.NET Core, który umożliwia tworzenie kontrolerów, routingu oraz automatyczną serializację i deserializację danych JSON
...
\paragraph{Struktura logiki frontendowej - autentykacja użytkowników}
\begin{center}
    \includegraphics[width=\textwidth]{figures/frontend-auth-structure.png}
    \captionof{figure}{Diagram struktury logiki frontendowej dotyczącej autentykacji użytkowników}
\end{center}

\paragraph{Struktura logiki frontendowej - Games}
\begin{center}
    \includegraphics[width=\textwidth]{figures/frontend-games-structure.png}
    \captionof{figure}{Diagram struktury logiki frontendowej dotyczącej gier}
\end{center}

\paragraph{Struktura logiki frontendowej - Mods}
\begin{center}
    \includegraphics[width=\textwidth]{figures/frontend-mods-structure.png}
    \captionof{figure}{Diagram struktury logiki frontendowej dotyczącej modyfikacji}
\end{center}

\paragraph{Struktura logiki frontendowej - User Profile}
\begin{center}
    \includegraphics[width=\textwidth]{figures/frontend-user-profile-structure.png}
    \captionof{figure}{Diagram struktury logiki frontendowej dotyczącej profili użytkowników}
\end{center}
...
\subsection{Endpointy związane z autoryzacją użytkownika}
\begin{center}
    \includegraphics[width=\textwidth, keepaspectratio]{figures/swagger-user-access.png}
    \captionof{figure}{Sekcja w Swaggerze pokazująca endpointy w kontrolerze UserAccess}    
\end{center}

\subsection{Rejestracja przy pomocy formularza}
\begin{lstlisting}[caption=Kontroler UserAccessController: endpoint SendRegisterForm, label=lst:sendregisterform]
    /// <summary>
...
Endpoint \texttt{SendRegisterForm} \ref{lst:sendregisterform} realizuje rejestrację użytkownika przy pomocy wypełnionego przez użytkownika formularza do rejestracji. Formularz ten składa się z 4 elementów typu \texttt{string} - UserName (nazwa użytkownika), Email, Password i ConfirmPassword (hasło oraz jego kopia, która powinna być identyczna). Walidacja polega na sprawdzeniu, czy:

\begin{itemize}
    \item email jest poprawny (wykorzystywany jest do tego \texttt{EmailHelper}, który korzysta z \texttt{EmailAddressAttribute} zaczerpnięty z \texttt{System.ComponentModel.DataAnnotations})
    \item hasło nie jest za krótkie oraz czy jest one identyczne jak \texttt{string} ConfirmPassword
    \item nazwa użytkownika jest między sześcioma a trzydziestoma znakami
    \item żaden z powyższych \texttt{string}ów nie jest pusty.
\end{itemize}

\subsection{Logowanie przy pomocy emaila oraz hasła}

\begin{lstlisting}[caption=Kontroler UserAccessController: endpoint SendLoginForm, label=lst:sendloginform]
    /// <summary>
...
\begin{lstlisting}[caption=Serwis AuthenticationService: metoda GenerateJwtToken, label=lst:generatejwttoken]
...
Powyższy fragment kodu \ref{lst:sendloginform} przedstawia implementację endpointu logowania użytkownika do aplikacji Candlelight za pomocą emaila oraz hasła. Korzysta on z serwisu AuthenticationService aby zweryfikować przesłane dane a następnie zalogować użytkownika i zwrócić token JWT. W aplikacji Candlelight token JWT jest używany do uwierzytelniania użytkownika przy każdym kolejnym żądaniu do zabezpieczonych zasobów API, zapewniając tym samym mechanizm sesji bezstanowej oraz kontrolę dostępu do danych zgodnie z tożsamością zalogowanego użytkownika
...
\begin{center}
    \includegraphics[width=\textwidth, keepaspectratio]{figures/candlelight-frontpage.png}
\end{center}
...
\subsection{Steam}
\subsubsection{Rejestracja oraz logowanie za pomocą konta Steam}
\begin{lstlisting}
...
Interfejs użytkownika aplikacji korzysta z biblioteki Angular Material \cite{angularmaterial2025}, która domyślnie integruje się z usługą Google Fonts (CDN Google Fonts \nolinkurl{fonts.googleapis.com}, \nolinkurl{fonts.gstatic.com})  w celu ładowania czcionki Roboto \cite{roboto2025} oraz zestawu ikon Material Icons \cite{materialicons2025}. Dzięki temu możliwe było uzyskanie spójnego i nowoczesnego wyglądu aplikacji przy minimalnym nakładzie konfiguracji.

Alternatywą dla ładowania czcionek i ikon z Google Fonts jest ich lokalne hostowanie w katalogu statycznym aplikacji oraz ręczne dołączenie ich za pomocą reguł @font-face w plikach stylów. Takie rozwiązanie pozwala uniknąć zależności od zewnętrznych serwerów oraz może być wymagane w środowiskach z ograniczonym dostępem do Internetu (intranety, sieci zamknięte).

W przypadku projektu Candlelight nie było konieczności stosowania lokalnej wersji, ponieważ aplikacja nie przetwarza danych szczególnie wrażliwych na poziomie frontendowym, środowisko produkcyjne nie ogranicza połączeń z Internetem, a korzystanie z CDN Google Fonts zapewnia lepszą wydajność dzięki globalnej dystrybucji oraz umożliwia cacheowanie czcionek w przeglądarce użytkownika, co przekłada się na szybsze ładowanie interfejsu i mniejsze zużycie zasobów.

Dodatkowo, użycie domyślnych linków do czcionek i ikon rekomendowanych przez Angular Material zapewnia pełną kompatybilność z komponentami biblioteki bez potrzeby ręcznej konfiguracji i dostosowywania stylów, co oszczędziło niepotrzebnej pracy i straty czasu.
